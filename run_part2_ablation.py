...
from io_utils import read_json, write_summary_csv
...
def main():
    ...
    parser.add_argument("--vary-seeds", action="store_true", help="Use seed+i for each variant. Default keeps the same seed for cleaner comparisons.")
    ...
    for i, variant in enumerate(variants):
        ...
        variant_seed = args.seed + i if args.vary_seeds else args.seed
        if args.skip_existing and (out_dir / "summary.json").exists():
            print(f"Reusing existing result: {out_dir.relative_to(PROJECT_DIR)}")
        else:
    ...
    write_summary_csv(comparison_csv, comparison_rows)
